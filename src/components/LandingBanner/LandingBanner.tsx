...
export function LandingBanner() {
  ...
  return (
    <main className="flex flex-1">
      <section className="flex w-full flex-1 border-t border-secondary bg-white px-4 text-black shadow-[0_16px_45px_-24px_rgba(15,23,42,0.18)] sm:border-y sm:px-6 lg:px-8 dark:border-secondary-dark dark:bg-secondary-dark/70 dark:text-teal-100 dark:shadow-[0_16px_45px_-24px_rgba(16,185,129,0.25)]">
          ...
          <div className="mt-6 flex w-full flex-row items-center justify-center gap-3">
            {NavigationData.filter((item) => item.url !== HOME_URL).map((item, index) => (
              <div key={`${index}-landing-banner-link`}>
                <Link
                  href={item.url}
                  className="inline-flex items-center rounded-full border border-secondary bg-white px-4 py-2 text-xs font-semibold text-black transition hover:bg-secondary dark:border-teal-500/80 dark:bg-secondary-dark dark:text-teal-100 dark:hover:bg-teal-900"
                >
                  {tNav(item.id)}
                </Link>
              </div>
